...
Chiaramente, non tutte le triple possono essere sensatamente considerati poteri, in quanto per alcuni sottoinsiemi $C$ il significato semantico di $a$ cozzerebbe. Nel caso dell'azione sopra considerata, avrebbe poco senso ad esempio considerare $(i,a, \lbrace i\rbrace)$, in quanto signicherebbe che "$i$ può andare a cena in 25 dicembre 2026 con se stesso", che ha poco senso (se non volendo essere proprio laschi con il significato delle parole). 
...
Intanto, definiamo per ogni $j \in I$, $P_j:=\lbrace (i,a, C) \in P | i=j\rbrace$, ossia l'insieme di tutti i possibili poteri a cui potrebbe accedere $j$.
...
Definiamo per ogni $j \in I$, $V_j=\lbrace(i,a,C) \in P | j \in C\rbrace$, ossia l'insieme di tutti i possibili poteri che può subire $j$.
...
Infine definiamo $P_i^s := \lbrace x \in P_i| x \in r(s)\rbrace$. 
Definiamo anche $V_i^s:=\lbrace x \in V_i | x \in r(s)\rbrace$.
...
**Definizione** Diciamo che un agente $i$ ha *una possibile pretesa trasformativa non coattiva* se esiste $s \in S$ tale che $P_i^{s_0} \leq^p_i P_i^s$, ed inoltre per ogni $j \in I \setminus \lbrace i \rbrace$ si ha che $V_j^{s_0}\leq^a_j V_j^s$.

**Definizione** Diciamo che un agente $i$ ha *una pretesa trasformativa non coattiva* se esiste $s \in S$ tale che $P_i^{s_0} \leq^p_i P_i^s$, per ogni $j \in I \setminus \lbrace i\rbrace$ si ha che $V_j^{s_0}\leq^a_j V_j^s$ ed infine $s_0 <_i^s s$.
